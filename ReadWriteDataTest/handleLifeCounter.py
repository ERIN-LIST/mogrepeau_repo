...
from Control.GPCVariablesConfig import GPC_StateVars
...
class MPCzLifeJob(jobNormal):
    ...
    def doRun(self,fsm):
        ...
        LData = self.LifeOPC
        LData.readOPC()
        zL = LData.opcVarsDict[self.zLifeVN]
        if zL.value not in [None, 0]:
            self.GPC_zLife = zL.value
...
class dummy():
    def __init__(self):
        self.opcVarsDict = {MPCzLifeJob.zLifeVN: opcVar(MPCzLifeJob.zLifeVN,1,'Good','2012-10-01 12:00:00')}
