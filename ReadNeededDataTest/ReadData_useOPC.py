...
from Control.GPCVariablesConfig import GPC_Stations, GPC_SysVars

class AlgData_OPC(object):

    Variables = {'OPC_Group':'GPCSysVariables' }
    Variables.update(GPC_SysVars)
    writeLockLimit = 10
    TimeoutErrorLimit = 15

    def __init__(self, opcserver='OPC.SimaticHMI.CoRtHmiRTm.1',
    ...
    def _init_OPC(self, opcserver='OPC.SimaticHMI.CoRtHmiRTm.1', opcclientName='OPCClient.Algo'):
        try:
        ...
        if opcserver in opc.servers():
            ...
            self.OPCCloseNeeded = False #By default suppose the close is not needed.
            try:
            ...
            if self.Variables.has_key('OPC_Group'):
                ...
                for sti in self.Variables.keys():
                    ...
                    vmapi = dict([(vi['OPC'],sti+'.'+vi['GPC']) for vi in self.Variables[sti]])
                    opcVMap.update(vmapi)
                    opcVars.update(vmapi.keys())
                opcServVars.update(opc.list(flat=True))
                opcVMap.update(dict([reversed(i) for i in opcVMap.items()]))
            else:
                opcGrp = self.Variables.keys()[0]
                opcVars = set([vi for vi in self.Variables[opcGrp]])
                opcServVars = set(opc.list(flat=True))
    ...
    def readOPC(self,opcVars=None,_recN=0):
        ...
        try:
            ...
            try:
                ...
                opcIn = self.opc.read(opcVars)#Gsc check if options needed: size=25, pause=10
                self._close_OPC()
    ...
    def _writeToOPC(self,gpcVars):
        ...
        try:
            ...
            try:
                self._connect_OPC()
                WStatus = self.opc.write(opcTVPairs) #Gsc check if options needed: size=2, pause=100
                sleep(0.2)
                self._close_OPC()
